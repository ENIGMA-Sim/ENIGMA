 ...
 *
 * This is the generic mobility template built as `mobility_test_app`
 * (see CMakeLists.txt). Use case 7 (../07_mobility_france_trains/
 * mobility_test.cpp) keeps an identical copy for its own self-contained
 * README/run.sh walkthrough — both use cases run the same binary, only the
 * platform XML and GPS traces passed on the command line differ.
 *
 * Shows how to:
